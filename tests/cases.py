...
real_root_cases = [
    (x, -1.0, 1.0, [0.0]),
    (x - 0.3, -1.0, 1.0, [0.3]),
    ((x + 0.5) * (x - 0.5), -1.0, 1.0, [-0.5, 0.5]),
    ((x + 0.7) * (x - 0.2) * (x - 0.9), -1.0, 1.0, [-0.7, 0.2, 0.9]),
    (sp.sin(sp.pi * x), -2.5, 2.5, [-2.0, -1.0, 0.0, 1.0, 2.0]),
    (sp.cos(sp.pi * x), -2.0, 2.0, [-1.5, -0.5, 0.5, 1.5]),
    ((x - 1) * (x - 2), 0.0, 3.0, [1.0, 2.0]),
]

complex_root_cases = [
    (sp.exp(sp.I * sp.pi * x) - 1, -3.5, 3.5, [-2.0, 0.0, 2.0]),
    (sp.exp(sp.I * sp.pi * x) + 1, -3.5, 3.5, [-3.0, -1.0, 1.0, 3.0]),
    ((1 + 2j) * (x - 0.3) * (x + 0.6), -1.0, 1.0, [-0.6, 0.3]),
]

real_extrema_cases = [
    (x**2, -1.0, 1.0, [0.0]),
    ((x - 0.3) ** 2, -1.0, 1.0, [0.3]),
    (x**3 - 3 * x, -2.0, 2.0, [-1.0, 1.0]),
    (x**4 - 2 * x**2, -2.0, 2.0, [-1.0, 0.0, 1.0]),
    (sp.cos(sp.pi * x), -2.5, 2.5, [-2.0, -1.0, 0.0, 1.0, 2.0]),
    (sp.sin(sp.pi * x), -2.0, 2.0, [-1.5, -0.5, 0.5, 1.5]),
    ((x - 1) * (x - 3), 0.0, 4.0, [2.0]),
]

complex_extrema_cases = [
    ((2 - 1j) * (x**3 - 3 * x), -2.0, 2.0, [-1.0, 1.0]),
    (sp.exp(sp.I * sp.pi * x) / (sp.I * sp.pi) - x, -3.5, 3.5, [-2.0, 0.0, 2.0]),
    (sp.exp(sp.I * sp.pi * x) / (sp.I * sp.pi) + x, -3.5, 3.5, [-3.0, -1.0, 1.0, 3.0]),
]
